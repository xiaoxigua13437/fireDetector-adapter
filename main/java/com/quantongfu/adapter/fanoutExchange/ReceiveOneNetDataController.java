...
@Controller
@RequestMapping("/receiveOneNetDataController")
public class ReceiveOneNetDataController {

    private static String token ="abc";//用户自定义token和OneNet第三方平台配置里的token一致
    private static String aeskey ="";//aeskey和OneNet第三方平台配置里的token一致
    private static Logger logger = LoggerFactory.getLogger(ReceiveOneNetDataController.class);
    @Autowired
    ...
    public String receive(@RequestBody String body) throws NoSuchPaddingException, InvalidKeyException, NoSuchAlgorithmException, IllegalBlockSizeException, BadPaddingException, InvalidAlgorithmParameterException {
        ...
        for(int i = 0; i < jsonObject.getJSONArray("msg").size(); i++){
            ...
            rabbitTemplate.convertAndSend("fanoutExchange","abcd.ee",receiveData);

            logger.info("fanoutExchange data:" + rabbitTemplate);
        }
